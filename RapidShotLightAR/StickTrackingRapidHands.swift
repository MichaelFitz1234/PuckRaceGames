...
class StickTrackingRapidHands: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate{
    ...
    fileprivate func lightUpRapid(numberAlreadylit: Int){
            var randomInt = Int.random(in: 0..<4)
            while(randomInt == numberAlreadylit){
                randomInt = Int.random(in: 0..<4)
            }
            RapidHandsViews[randomInt].backgroundColor = .red
            whichRectangle = randomInt
        
    }
    fileprivate func addConfigs(){
        //print("method1")
        RapidHands.append(generateRapidHands(startX: 200, startY: 50))
...
//
    ...
    @objc private func readForStuff(){
        if(firstDetect == 1){
          ...
          mode = 2
          addConfigs()
          lightUpRapid(numberAlreadylit: 5)
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        DispatchQueue.main.async { [self] in
            ...
            //view.addSubview(DRAW(frame: checkFrame))
                if(whichRectangle != -1){
                if(checkFrame.intersects(self.RapidHandsViews[self.whichRectangle].frame)){
                        self.RapidHandsViews[self.whichRectangle].backgroundColor = .yellow
                        //print(self.RapidHandsViews[self.whichRectangle])
                        self.lightUpRapid(numberAlreadylit: self.whichRectangle)
                        count = count + 1
                        countLabel.text = "Targets Hit " + String(count) + " / " + String(numberOfTargets)
