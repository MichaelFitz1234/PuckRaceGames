...
class RandomStickTracking: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate,MCBrowserViewControllerDelegate, MCNearbyServiceAdvertiserDelegate{
    ...
    fileprivate func addConfigs(){
        ...
        let randomIntX = Int.random(in: 200..<400)
        let randomIntY = Int.random(in: 200..<500)
        RapidHands.append(generateRapidHands(startX: randomIntX, startY: randomIntY))
...
//
    ...
    fileprivate func generateRapidHands(startX: Int, startY: Int) -> CGRect{
    ...
    @objc private func readForStuff(){
        if(firstDetect == 1){
          ...
          view.addSubview(countLabel)
            ...
            countLabel.text =  "Targets Hit " + String(count) + " / " + String(numberOfTargets)
...
//                self.updateRight(numberMoved: 2)
//            }
//        }
//    }
   ...
   // var request: VNRequest
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        //pixelBuffer.transform = CGAffineTransform(scaleX: -1, y: -1)
        ...
        DispatchQueue.main.async { [self] in
            ...
            let point = self.undoVideoFromRatio(point:CGPoint(x: 1.0 - (tracked?.x ?? 0.0) , y: tracked?.y ?? 0.0), rect: self.view.frame)
                //print(point)
                ...
                if(checkFrame.intersects(self.RapidHandsViews[0].frame)){
                    ...
                    if(count == numberOfTargets){
                        self.delegate?.gameEnded(numberOfShots: numberOfTargets, timeTaken: time)
                    }
                    }
                }
            }
    ...
    func setupVideoOutputView(_ videoOutputView: UIView) {
        ...
        videoOutputView.transform = CGAffineTransform(scaleX: 1, y: -1)
